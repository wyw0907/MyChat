...
//const u8 TX_ADDRESS[TX_ADR_WIDTH]={0x30,0x31,0x32,0x33,0x34}; //发送地址
//const u8 RX_ADDRESS[RX_ADR_WIDTH]={0x30,0x31,0x32,0x33,0x34}; //发送地址		
//初始化24L01的IO口
...
//*pBuf:数据指针
//返回值,此次读到的状态寄存器值
u8 NRF24L01_Write_Buf(u8 regaddr, const u8 *pBuf, u8 datalen)
{
	u8 status,u8_ctr;	    
 	Clr_NRF24L01_CSN;                                    //使能SPI传输
  	status = SPI_ReadWriteByte(regaddr);                //发送寄存器值(位置),并读取状态值
